<template>
  <div class="flex flex-col items-center justify-center gap-6">
    ...
    <div class="flex flex-col items-center justify-center w-full" v-if="prizes">
      ...
      <ModalComponent v-if="selectedPrizeToDelete" :isOpen="isModalOpened" @modal-close="closeDeleteModal"
        name="first-modal">
        <template #header>Delete {{ selectedPrizeToDelete.name }}?</template>
        <template #content>Are you sure you want to delete <span class="font-bold">{{
          selectedPrizeToDelete.name }}</span>?
          Prizes related will be
          deleted.</template>
        <template #footer>
          <button @click="handleDeletePrize"
