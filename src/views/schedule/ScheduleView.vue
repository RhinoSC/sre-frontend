<template>
  <div class="flex flex-col items-center justify-center gap-6">
    ...
    <div class="flex flex-col items-center justify-center w-full">
      ...
      <ModalComponent v-if="selectedScheduleToDelete" :isOpen="isModalOpened" @modal-close="closeDeleteModal"
        name="first-modal">
        <template #header>Delete {{ selectedScheduleToDelete.name }}?</template>
        <template #content>Are you sure you want to delete <span class="font-bold">{{
          selectedScheduleToDelete.name }}</span>?
          Runs related will be
          deleted.</template>
        <template #footer>
          <button @click="handleDeleteSchedule"
