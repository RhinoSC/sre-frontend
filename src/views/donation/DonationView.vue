<template>
  <div class="flex flex-col items-center justify-center gap-6">
    ...
    <div class="flex flex-col items-center justify-center w-full" v-if="donations">
          ...
          <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700" v-for="donation in filteredDonations"
            ...
            <td class="px-6 py-4">
              <div class="flex flex-row items-center justify-center gap-2">
                <RouterLink :to="`/donations/edit/${donation.id}`">
                ...
                <button @click="openDeleteModal(donation)">
                  <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                    <path fill-rule="evenodd"
      ...
      <ModalComponent v-if="selectedDonationToDelete" :isOpen="isModalOpened" @modal-close="closeDeleteModal"
        name="first-modal">
        <template #header>Delete {{ selectedDonationToDelete.name }}?</template>
        <template #content>Are you sure you want to delete <span class="font-bold">{{
          selectedDonationToDelete.name }}</span>?
          Donations related will be
          deleted.</template>
        <template #footer>
          <button @click="handleDeleteDonation"
...
import { currencyFormat, MStoStringTime } from '@/utils/strings'
import { apiDeleteDonation, apiGetDonations } from '@/api/donation/donation';
...
const closeDeleteModal = () => {
  ...
  selectedDonationToDelete.value = undefined
};
...
const handleDeleteDonation = async () => {
  try {
    if (!selectedDonationToDelete.value) {
      throw new Error("there is no donation");
    }
    const response: APIResponse<DonationWithoutBidDetails[]> = await apiDeleteDonation(selectedDonationToDelete.value.id)
    donations.value = response.data
    closeDeleteModal()
    window.location.reload();
  } catch (error) {
    console.error("Failed to delete donation:", error);
    alert("There was an error deleting the donation. Please try again.");
  }
}

// const donations = [{ "event_id": "event1", "name": "event 1 name", "donations": [], "start_time_mili": "12:00 - 22-09-2024", "end_time_mili": "12:00 - 22-09-2024" }]
