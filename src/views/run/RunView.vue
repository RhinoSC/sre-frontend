<template>
  <div class="flex flex-col items-center justify-center gap-6">
    <div class="">
      <div class="flex flex-col items-center justify-center gap-6">
        <h1 class="text-4xl font-bold text-center text-violet-600">Runs</h1>
        <p class="text-xl">Total runs: {{ runs?.length }}</p>
        <RouterLink class="h-12 text-center w-60" to="/runs/new">
          <div
    ...
    <div class="flex flex-col items-center justify-center w-full" v-if="runs">
      <table class="w-full text-left table-auto rtl:text-right">
        ...
        <tbody>
          <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700" v-for="run in runs" :key="run.id">
            ...
            <td class="px-6 py-4">
              <span class="pr-2">{{ run.start_time_mili === 0 ? "-" : new
                Date(run.start_time_mili).toLocaleString('es-ES', {
                  day: '2-digit',
                  month: 'short',
                  year: 'numeric',
                  hour: 'numeric',
            ...
            <td class="flex flex-row items-center justify-start gap-2 px-6 py-4">
              <RouterLink :to="`/runs/edit/${run.id}`">
              ...
              <button @click="openDeleteModal(run)">
                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                  width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                  <path fill-rule="evenodd"
      ...
      <ModalComponent v-if="selectedRunToDelete" :isOpen="isModalOpened" @modal-close="closeDeleteModal"
        name="first-modal">
        <template #header>Delete {{ selectedRunToDelete.name }}?</template>
        <template #content>Are you sure you want to delete <span class="font-bold">{{
          selectedRunToDelete.name }}</span>?
          Runs related will be
          deleted.</template>
        <template #footer>
          <button @click="handleDeleteRun"
...
import { apiDeleteRun, apiGetRuns } from '@/api/run/run';
...
const handleDeleteRun = async () => {
  try {
    if (!selectedRunToDelete.value) {
      throw new Error("there is no run");
    }
    const response: APIResponse<Run[]> = await apiDeleteRun(selectedRunToDelete.value.id)
    runs.value = response.data
    closeDeleteModal()
    window.location.reload();
  } catch (error) {
    console.error("Failed to delete run:", error);
    alert("There was an error deleting the run. Please try again.");
  }
}

// const runs = [{ "event_id": "event1", "name": "event 1 name", "runs": [], "start_time_mili": "12:00 - 22-09-2024", "end_time_mili": "12:00 - 22-09-2024" }]
const runs = ref<Run[]>()
