<template>
  <div class="flex flex-col items-center justify-center gap-6" v-if="oldRun">
    <div class="">
      <div class="flex flex-col items-center justify-center gap-6">
        <h1 class="text-4xl font-bold text-center text-violet-600">Edit Run {{ oldRun.name }}</h1>
      </div>
    </div>
    <div class="w-full" v-if="newRun">
        ...
        <div class="max-w-3xl">
          ...
          <div class="flex flex-wrap mb-6 -mx-3">
            ...
            <div class="w-1/3 px-3">
              ...
              <CategorySelect :category_id="newRun.run_metadata.twitch_game_id"
                @selectCategory="selectCategory($event)"></CategorySelect>
            </div>
...
import { apiCreateRun, apiGetRunByID, apiUpdateRun } from '@/api/run/run';
...
const oldRun = ref<Run>()
const newRun = ref<Run>()
...
const handleUpdateRun = async () => {
  try {
    ...
    //   newRun.value.schedule_id = selectedSchedule.value.id
    // }
...
const convertMSToSetupTime = (milis: number) => {
  const totalMinutes = Math.floor(milis / (1000 * 60));
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;
...
const convertMSToRunTime = (milis: number) => {
  const totalMinutes = Math.floor(milis / (1000 * 60));
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;

  runTime.value.hours = hours
  runTime.value.minutes = minutes
}

const handleGetRunById = async () => {
  try {
    const id = Array.isArray(route.params.id) ? route.params.id[0] : route.params.id;
    if (!id) throw new Error("Error getting id");

    const response: APIResponse<Run> = await apiGetRunByID(id)
    oldRun.value = response.data
