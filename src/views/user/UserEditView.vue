<template>
  <div class="flex flex-col items-center justify-center gap-6" v-if="oldUser">
    <div class="">
      <div class="flex flex-col items-center justify-center gap-6">
        <h1 class="text-4xl font-bold text-center text-violet-600">Edit User {{ oldUser.username }}</h1>
      </div>
    </div>
    <div class="w-full">
      <VForm @submit="handleUpdateUser" class="flex flex-col items-center justify-center" :validation-schema="schema">
...
import { apiGetUserByID, apiUpdateUser } from '@/api/user/user';

const router = useRouter()
const route = useRoute()

const oldUser = ref<User>({ id: "", name: "", username: "", socials: { twitch: "", twitter: "", youtube: "", facebook: "" } as Socials })
const newUser = ref<User>({ id: "", name: "", username: "", socials: { twitch: "", twitter: "", youtube: "", facebook: "" } as Socials })
...
const handleGetUserById = async () => {
  try {
    ...
    const response: APIResponse<User> = await apiGetUserByID(id)
    oldUser.value = response.data
    newUser.value = JSON.parse(JSON.stringify(response.data))
  } catch (error) {
